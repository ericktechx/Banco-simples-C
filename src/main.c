...
typedef struct {
	...
	float saldo;
	float limite;
	float saldoTotal;  // saldoTotal é sempre calculado como saldo + limite
} Conta;
...
// Arrays globais que funcionam como o "banco de dados" em memória do programa.
// Limite fixo de 50 contas.
...
int main() {
	...
	// Define o locale para português, resolvendo problemas com acentuação no terminal Windows
...
void infoCliente(Cliente cliente){
	// strtok com "\n" remove o newline que o fgets deixa no final das strings
...
void criarConta(){
	...
	// Captura a data atual do sistema para usar como data de cadastro
	char dia[3];
	char mes[3];
	char ano[5];
	...
	// Formata com zero à esquerda quando necessário (ex: "01" em vez de "1")
	if (tm.tm_mday < 10) {
	...
	// tm_mon começa em 0 (janeiro = 0), por isso o +1
	if ((tm.tm_mon + 1) < 10) {
	...
	// tm_year conta a partir de 1900, por isso o +1900
	sprintf(ano, "%d", tm.tm_year + 1900);
	
	// Monta a string da data no formato DD/MM/AAAA
	...
	// Associa o cliente à conta e inicializa saldo e limite zerados
...
// Recalcula o saldo total com base no saldo atual e no limite disponível.
// Deve ser chamada sempre que saldo ou limite forem alterados.
float atualizaSaldoTotal(Conta conta){
	return conta.saldo + conta.limite;
}

// Percorre o array global buscando uma conta pelo número.
// Retorna uma Conta não inicializada se não encontrar — checar sempre se conta.numero == numero após chamar.
Conta buscarContaPorNumero(int numero){
	Conta c;
	if (contador_contas > 0) {
		for (int i = 0; i < contador_contas; i++){
		...
		for (int i = 0; i < contador_contas; i++) {
			if (contas[i].numero == conta.numero) {
				if (contas[i].saldo >= valor) {
					...
					// Saldo insuficiente: usa o limite para cobrir o restante.
					// "restante" será negativo, reduzindo o limite disponível.
...
void transferir(Conta conta_origem, Conta conta_destino, float valor){
	if (valor > 0 && conta_origem.saldoTotal >= valor) {
		// Loop duplo para localizar origem (co) e destino (cd) no array global
				...
				for(int cd = 0; cd < contador_contas; cd++) {
					if (contas[cd].numero == conta_destino.numero) {
						if (contas[co].saldo >= valor) {
							...
							// Mesma lógica do saque: cobre o restante com o limite da conta origem
